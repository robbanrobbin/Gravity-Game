	...
	public void update(double delta) {
		...
		for (Body body1 : bodies) {
			...
			for (Body body2 : bodies) {
				...
				if (coll = Collider.collision(
						body1.collider.tempAdd(body1.velX * Gravity.getGame().delta,
					...
					if (relativeVelocity < 30000 && body1.getMass() >= body2.getMass()) {
						...
						if (body2.getMass() <= 10) {
							...
							// body2.velX * MathUtils.randomDouble(random, 0.05,
							// 0.1) * 2,
							// body2.velY * MathUtils.randomDouble(random, 0.05,
							// 0.1) * 2, 0.7f, new Color(0.3f, 0.3f, 0.3f));
							...
							// for(int p = 0; p < 1; p++) {
							// Particle part = new Particle(body2.x, body2.y,
							// body2.velX * MathUtils.randomDouble(random, 0.001,
							// 0.06) * 2,
							// body2.velY * MathUtils.randomDouble(random, 0.001,
							// 0.06) * 2, 0.7f, new Color(0.7f, 0.7f, 0.7f));
							...
							// 0.01) * 2,
							// body2.velY * MathUtils.randomDouble(random, 0.001,
							// 0.01) * 2, 0.7f, new Color(1f, 1f, 1f));
						...
						if ((body1.getMass() >= body2.getMass())) {
							...
							for (int i = 0; i < pieces; i++) {
								Body body = new Body(
										body2.x + MathUtils.randomDouble(random, -body2.collider.radius,
								...
								// MathUtils.getAngle(body2, body1) +
								// MathUtils.randomDouble(random, -60, 60),
								// velocity1);
								addBody(body);
							}
							removeBody(body2);
				...
				if (!coll && body1.hasAtmosphere()) {
					coll = Collider.collision(
							body1.atmosphere.collider.tempAdd(body1.velX * Gravity.getGame().delta,
									body1.velY * Gravity.getGame().delta),
							body2.collider.tempAdd(body2.velX * Gravity.getGame().delta,
									body2.velY * Gravity.getGame().delta));
					if (coll) {
